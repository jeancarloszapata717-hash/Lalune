...
export default function Cart() {
  const cartItems = [
    {
      id: "1",
      product: MOCK_PRODUCTS.find(p => p.id === 'bombs-choco')!,
      packType: 'PROMO DE 2',
      flavors: ['PISTACHO', 'CRICRI'],
      quantity: 1,
      unitPrice: 13.00, // 10 base + 2 pistacho + 1 cricri
    },
    {
      id: "2",
      product: MOCK_PRODUCTS.find(p => p.id === 'cookie-red-velvet')!,
      quantity: 2,
      unitPrice: 8.00,
    }
  ];
  ...
  const total = subtotal; // Assuming no shipping costs for this simplistic view yet
                  ...
                  <div className="flex-1 flex flex-col justify-between py-0.5 md:py-1">
                    <div>
                      <div className="flex justify-between items-start mb-0.5">
                        ...
                        <button className="text-brand-dark/40 hover:text-red-500 transition-colors p-1 absolute right-2 top-2 md:relative md:right-0 md:top-0">
                    ...
                    <div className="flex items-center justify-between mt-2 md:mt-3">
                       {/* Qty Control */}
                       <div className="flex items-center border border-brand-dark/20 rounded-full h-7 sm:h-8 md:h-10 w-20 sm:w-20 md:w-24 bg-white">
                         <button className="w-6 sm:w-6 md:w-8 h-full flex items-center justify-center text-brand-dark hover:text-brand-hotpink">
                           <Minus size={12} strokeWidth={2.5} className="md:w-[14px] md:h-[14px]" />
                         </button>
                         <span className="flex-1 text-center font-sans font-bold text-xs sm:text-xs md:text-sm">{item.quantity}</span>
                         <button className="w-6 sm:w-6 md:w-8 h-full flex items-center justify-center text-brand-dark hover:text-brand-hotpink">
                           <Plus size={12} strokeWidth={2.5} className="md:w-[14px] md:h-[14px]" />
                         </button>
                       </div>
                      
                      <span className="font-sans font-black text-sm sm:text-base md:text-lg text-brand-dark pl-2">${(item.unitPrice * item.quantity).toFixed(2)}</span>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="lg:col-span-1">
              <div className="bg-brand-dark text-brand-cream p-6 md:p-8 rounded-[2rem] md:sticky md:top-32 shadow-[8px_8px_0px_0px_rgba(213,63,140,1)]">
                ...
                <div className="flex justify-between items-center mb-6 md:mb-8">
                  ...
                  <span className="font-bold text-xl md:text-2xl text-brand-hotpink">${total.toFixed(2)}</span>
                </div>
