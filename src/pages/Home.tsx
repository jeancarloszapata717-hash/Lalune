...
import { MOCK_PRODUCTS, LOCATIONS } from '../data/mockData';
...
export default function Home() {
  const recentPurchases = MOCK_PRODUCTS.filter(p => ['bombs-choco', 'pavlova-fiesta', 'sweet-bites-box', 'cookie-red-velvet'].includes(p.id));
  
  // Carousel State for Novedades (Allows for multiple promos if expanded)
  const promoImages = ["/ACTION.png"];
    ...
    <div className="w-full">
      ...
      <section className="pt-20 md:pt-24 pb-10 md:pb-12 bg-brand-cream text-brand-dark">
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          ...
          <div className="w-full relative mx-auto max-w-5xl rounded-2xl md:rounded-3xl overflow-hidden flex items-center justify-center">
      ...
      <section className="py-16 md:py-32 bg-brand-cream border-t border-brand-dark/10">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 text-center">
          <span className="text-brand-hotpink font-bold text-[10px] md:text-xs tracking-[0.3em] uppercase mb-4 block">Ubicaciones</span>
          <h2 className="font-display font-black text-3xl sm:text-5xl md:text-6xl mb-10 md:mb-16 text-brand-dark uppercase tracking-tight">Visítanos</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
            {LOCATIONS.map((loc, idx) => (
              <motion.div 
                ...
                className="bg-white p-8 md:p-10 border-2 border-brand-dark/10 hover:border-brand-dark hover:shadow-[8px_8px_0px_0px_rgba(26,29,46,1)] transition-all duration-300 rounded-[20px] md:rounded-none"
              >
                <MapPin className="mx-auto mb-6 text-brand-hotpink" size={40} />
                <h3 className="font-display font-black uppercase text-2xl mb-3 text-brand-dark">{loc.city}</h3>
                <p className="font-sans font-medium text-brand-dark/80">{loc.address}</p>
              </motion.div>
            ))}
          </div>
        </div>
