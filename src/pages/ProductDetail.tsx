...
export default function ProductDetail() {
  ...
  const currentPrice = useMemo(() => {
    ...
    return product.basePrice + (selectedVariant ? selectedVariant.plusPrice : 0);
  }, [product, packType, selectedFlavors, selectedVariant]);
  ...
  const isReadyToAddToCart = useMemo(() => {
    ...
    return true;
  }, [product, packType, selectedFlavors]);
  ...
  const handleAddToCart = () => {
      ...
      console.log("Added to cart", { 
        ...
        flavors: product.hasPackOptions ? selectedFlavors.filter(Boolean).map(f => f?.name) : undefined,
        quantity, 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-16 items-start">
          ...
          <div className="flex flex-col py-0 md:py-4">
            ...
            <p className="font-sans font-black text-[22px] sm:text-3xl text-brand-dark mb-4 sm:mb-8">${currentPrice.toFixed(2)}</p>
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mb-2 mt-4">
              {/* Only show quantity selector if not 1 BOMB mode */}
              {(!product.hasPackOptions || packType === 'PROMO DE 2') && (
                <div className="flex items-center border-2 border-brand-dark rounded-full overflow-hidden h-12 md:h-14 sm:h-16 lg:h-20 w-full sm:w-32 lg:w-40 bg-white shadow-sm">
                  ...
                  <span className="flex-1 text-center font-sans font-bold text-base md:text-lg sm:text-xl">{quantity}</span>
                  <button 
              ...
              >
                ...
                <span className="font-sans font-bold text-xs md:text-base tracking-[0.2em] uppercase">
                  {product.hasPackOptions && !isReadyToAddToCart ? 'Selecciona Sabores' : 'Añadir al Carrito'}
                </span>
              </motion.button>
            </div>
